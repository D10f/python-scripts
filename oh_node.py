            ...
            try:
                for line in f:
                    ...
                    if lines == 1_000_000:
                        total_lines += lines
                        lines = 0
                        logging.debug(f'{total_lines} lines of code counted in {round(time.time() - start, 2)}s...')
            except UnicodeDecodeError as error:
                # TODO: error handling for binary formats
                pass

    logging.debug(f'{total_lines + lines} lines of code counted in {round(time.time() - start, 2)}s...')
...
def count_files(root_dir):
    ...
    logging.debug(f'{len(total_files)} files counted in {round(time.time() - start)}s...')
